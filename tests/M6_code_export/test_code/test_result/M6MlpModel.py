...
class M6MlpModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.layer_fc1 = nn.Linear(in_features=784, out_features=64)
        self.layer_relu = nn.ReLU()
        self.layer_dropout = nn.Dropout(p=0.25)
        self.layer_fc2 = nn.Linear(in_features=64, out_features=10)
    ...
    def forward(self, x):
        ...
        out_dropout = self.layer_dropout(outputs['relu'])
        outputs['dropout'] = out_dropout
        out_fc2 = self.layer_fc2(outputs['dropout'])
        outputs['fc2'] = out_fc2
        out_out = outputs['fc2']
        outputs['out'] = out_out
        return outputs['out']
...
MODEL_INPUTS = [{'id': 'input', 'shape': [784]}]

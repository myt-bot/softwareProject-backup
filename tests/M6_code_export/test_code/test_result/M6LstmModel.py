...
class M6LstmModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.layer_lstm = LSTMLayer(input_size=8, hidden_size=16, num_layers=1, bidirectional=True, return_sequences=False)
        self.layer_fc = nn.Linear(in_features=32, out_features=4)

    def forward(self, x):
        outputs = {}
        out_input = x
        outputs['input'] = out_input
        out_lstm = self.layer_lstm(outputs['input'])
        outputs['lstm'] = out_lstm
        out_fc = self.layer_fc(outputs['lstm'])
        outputs['fc'] = out_fc
        out_out = outputs['fc']
        outputs['out'] = out_out
        return outputs['out']
...
MODEL_INPUTS = [{'id': 'input', 'shape': [12, 8]}]

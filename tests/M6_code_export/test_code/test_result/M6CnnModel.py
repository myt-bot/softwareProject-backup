...
class M6CnnModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.layer_conv = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=3, stride=1, padding=1, padding_mode='zeros')
        self.layer_relu = nn.ReLU()
        self.layer_pool = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.layer_flat = nn.Flatten()
        self.layer_fc = nn.Linear(in_features=1568, out_features=10)
    ...
    def forward(self, x):
        ...
        out_relu = self.layer_relu(outputs['conv'])
        outputs['relu'] = out_relu
        out_pool = self.layer_pool(outputs['relu'])
        outputs['pool'] = out_pool
        out_flat = self.layer_flat(outputs['pool'])
        outputs['flat'] = out_flat
        out_fc = self.layer_fc(outputs['flat'])
        outputs['fc'] = out_fc
        out_out = outputs['fc']
        outputs['out'] = out_out
        return outputs['out']

...
class M6AddBranchModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.layer_left = nn.Linear(in_features=4, out_features=4)
        self.layer_right = nn.Linear(in_features=4, out_features=4)
        self.layer_merge_relu = nn.ReLU()

    def forward(self, x):
        outputs = {}
        out_input = x
        outputs['input'] = out_input
        out_left = self.layer_left(outputs['input'])
        outputs['left'] = out_left
        out_right = self.layer_right(outputs['input'])
        outputs['right'] = out_right
        out_merge_relu = self.layer_merge_relu(sum([outputs['left'], outputs['right']][1:], [outputs['left'], outputs['right']][0]))
        outputs['merge_relu'] = out_merge_relu
        out_out = outputs['merge_relu']
        outputs['out'] = out_out
        return outputs['out']

TRAIN_CONFIG = {'dataset_name': 'MNIST', 'epochs': 1, 'batch_size': 1, 'rate': 0.001, 'device': 'cpu', 'loss_fn': 'cross_entropy', 'optimizer': 'sgd', 'data_dir': '', 'artifacts_dir': ''}

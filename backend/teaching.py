...
_ERROR_SUGGESTION_RULES: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "reason": "一个节点接收多条分支时，系统需要知道如何把它们合并：逐元素相加、按维度拼接，还是做矩阵乘法。",
        "suggestions": [
            "用「合并运算」模块合并多条分支，并在右侧参数面板选择 add、concat 或 matmul 模式。",
            "add（逐元素相加）要求各输入 shape 完全一致；concat（拼接）需检查 dim；matmul（矩阵乘法）对输入顺序敏感，可用 ⬅ / ➡ 调整。",
            "如果并不需要多输入，删除多余连线，只保留一条输入路径。",
        ],
        "related_layers": ["Merge", "Add"],
        "related_parameters": ["merge", "dim", "order"],
    },
    {
